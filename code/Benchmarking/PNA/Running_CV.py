...
def run_CVtraining(target, fold, params, save_model=False):
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch >= num_mean_vals:
            val_list = []
            for val in Results['loss_internal-test'][-num_mean_vals:]:
                val_list.append(val)
            ...
            if avg < best_loss and intest_loss < best_loss:
                ...
                if save_model:
                    ...
                    with open(model_dir+'/trained_model/CV/deg_cv'+str(fold), 'wb') as dg:
                        pickle.dump(deg, dg)
                    torch.save(Model.state_dict(), model_dir+'/trained_model/CV/ST-onlyPNA_cv'+str(fold)+'.pt')
