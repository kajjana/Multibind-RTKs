...
def define_regression_model(n_tasks=1, graph_attention_layers = [8,8], param=erbB2_param, config=default_config, model_dir='/tmp'):
       return GATModel(n_tasks=n_tasks, graph_attention_layers=graph_attention_layers, n_attention_heads=param['n_attention_heads'], 
                               predictor_hidden_feats =  param['predictor_hidden_feats'], predictor_dropout = param['predictor_dropout'], number_atom_features = 30, batch_size=param['batch_size'],learning_rate=param['learning_rate'],
...
def train_and_validate(target, train, n_tasks, outdir, graph_attention_layers , param, num_epochs, pickle_file_name, valid=None, test=None, gpu=None):
    if gpu is None:
        import os
    ...
    model = define_regression_model(n_tasks, graph_attention_layers=graph_attention_layers,param=param, model_dir=model_dir, config=config)
    ...
    for l in range(0, num_epochs):
        ...
        if test is not None:
            try:
                ...
                if l >= num_mean_vals:
                  val_list = []
                  for val in monitor_list[-num_mean_vals:]: #total loss intest
                    val_list.append(val)
                ...
                if early_stopping_counter == early_stopping_iter:
                  ...
                  result_df.to_csv(save_dir+target+'/dataframe/cv/Result/Extest_fulltrain_GAT.csv', index=False)
                 
                  result = {'ytrue':train_ytrue,'yhat':train_yhat}
                  result_df = pd.DataFrame(result)
                  save_dir = '/share/galaxy/fahsai/NSCLC/Sin/single_task/single_task/'
                  result_df.to_csv(save_dir+target+'/dataframe/cv/Result/Train_fulltrain_GAT.csv', index=False)
